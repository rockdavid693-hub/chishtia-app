...
if st.session_state.current_page == "Home Pharmacy":
    ...
    symptoms_text = st.text_area("Describe Symptoms / Bemari Ki Details Likhein *")
    ...
    if search_input:
            ...
            for row in user_orders:
                ...
                <div style="border: 1px solid #ddd; padding: 15px; border-radius: 8px; margin-bottom: 10px; background-color: #fafafa;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <b>Order ID: #{row['order_id']}</b><span class="status-badge {status_class}">{row['status']}</span>
                    </div>
                    <p style="margin: 5px 0; font-size: 0.9rem; color: #555;"><b>Date:</b> {row['timestamp']}</p>
                    <p style="margin: 5px 0; font-size: 0.9rem; color: #333;"><b>Items:</b><br>{row['order_details']}</p>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("Is number/ID se koi order nahi mila.")
        ...
        if user_consults:
            for row in user_consults:
                ...
                <div style="border: 1px solid #ddd; padding: 15px; border-radius: 8px; margin-bottom: 10px; background-color: #fafafa;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <b>Consultation ID: {row['consultation_id']}</b><span class="status-badge {status_class}">{row['status']}</span>
                    </div>
                    <p style="margin: 5px 0; font-size: 0.9rem; color: #555;"><b>Date:</b> {row['timestamp']}</p>
                    <p style="margin: 5px 0; font-size: 0.9rem; color: #333;"><b>Symptoms:</b> {row['symptoms']}</p>
                    <div style="background-color: #f0fdf4; border-left: 4px solid #16a34a; padding: 10px; margin-top: 10px; border-radius: 4px;">
                        ...
                        {row['doctor_reply'] if row['doctor_reply'] else 'Babar Aziz sahib aap ki payment check karke jald doctor ka jawab yahan share karenge.'}
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if row['doctor_voice_reply'] and os.path.exists(row['doctor_voice_reply']):
                    st.write("🎤 **Doctor Audio Prescription / Jawab Ki Audio:**")
                    st.audio(row['doctor_voice_reply'])
        else:
            st.info("Is number/ID se koi consultation nahi mili.")
                ...
                for index, row in orders_df.iterrows():
                    with st.expander(f"Order {row['order_id']} - {row['customer_name']} [{row['status']}]"):
                        ...
                        if row['prescription_path'] and isinstance(row['prescription_path'], str) and os.path.exists(row['prescription_path']):
                            ...
                            else: st.image(row['prescription_path'], width=300)
                        ...
                        if col_del.button("🗑️ Delete Order Permanently (History Clear)", key=f"del_{row['order_id']}"):
                            ...
                            st.rerun()
            else: st.write("No product orders recorded.")
            ...
            if not cons_df.empty:
                for idx, row in cons_df.iterrows():
                    with st.expander(f"Consultation {row['consultation_id']} - {row['patient_name']} [{row['status']}]"):
                        ...
                        # ADVANCED FEATURE: OPTIONAL AUDIO VOICE FILE REPLY BY DOCTOR
                        ...
                        if col_c_up.button("Apply Operational Decision & Send Reply", key=f"c_btn_{row['consultation_id']}"):
                            ...
                            if dr_audio_file:
                                ...
                                with open(v_reply_path, "wb") as f: f.write(dr_audio_file.getbuffer())
                        ...
                        if col_c_del.button("🗑️ Delete Consultation Permanently", key=f"c_del_{row['consultation_id']}"):
                            ...
                            st.rerun()
            else: st.write("No consultation clinical bookings.")
